...
from tests.validation.check.row_level_result.utils.numeric_check_tests_cases import (
    is_greater_or_equal_to_tests_cases_parameterize,
)
...
class TestIsGreaterOrEqualThanUnit:
    @pytest.mark.parametrize(
        **is_greater_or_equal_to_tests_cases_parameterize(pass_outcome=True),
    )
    def test_pass(
        self,
    ...
    @pytest.mark.parametrize(
        **is_greater_or_equal_to_tests_cases_parameterize(pass_outcome=False),
    )
    def test_fail(
        self,
        check: RowLevelResultCheck,
