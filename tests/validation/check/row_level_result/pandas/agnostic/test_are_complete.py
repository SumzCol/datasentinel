...
from tests.validation.check.row_level_result.utils.agnostic_check_tests_cases import (
    are_complete_tests_cases_parameterize,
)
...
class TestAreCompleteUnit:
    @pytest.mark.parametrize(
        **are_complete_tests_cases_parameterize(pass_outcome=True),
    )
    def test_pass(
        self,
        ...
        result = check.are_complete(column=evaluated_column, id_columns=id_columns).validate(df)
    ...
    @pytest.mark.parametrize(
        **are_complete_tests_cases_parameterize(pass_outcome=False),
    )
    def test_fail(
        self,
        check: RowLevelResultCheck,
