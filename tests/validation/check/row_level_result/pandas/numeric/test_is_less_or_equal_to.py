...
from tests.validation.check.row_level_result.utils.numeric_check_tests_cases import (
    is_less_or_equal_to_tests_cases_parameterize,
)
...
class TestIsLessOrEqualToUnit:
    @pytest.mark.parametrize(
        **is_less_or_equal_to_tests_cases_parameterize(pass_outcome=True),
    )
    def test_pass(
        self,
        ...
        result = check.is_less_or_equal_to(
    ...
    @pytest.mark.parametrize(
        **is_less_or_equal_to_tests_cases_parameterize(pass_outcome=False),
    )
    def test_fail(
        self,
        check: RowLevelResultCheck,
